...
// ---------------------------------------------------------------------------
// small helper textures owned by this domain
// ---------------------------------------------------------------------------

/** Soft elliptical blob used for the ball's contact shadow. */
  ...
  if (!cache.has(key)) {
    ...
    for (let y = 0; y < S; y++) {
      for (let x = 0; x < S; x++) {
        ...
        // dense core with a fast, soft falloff — a real contact shadow, not a haze
        const a = Math.max(0, 1 - d) ** 1.25 * (1 - smoothstep(0.55, 1.0, d));
        const o = (y * S + x) * 4;
...
/** Longitudinal gradient for the motion trail: opaque at the head, gone at the tail. */
export function trailTexture() {
  const key = 'trail';
  if (!cache.has(key)) {
    const Wt = 64, Ht = 32;
    const data = new Uint8ClampedArray(Wt * Ht * 4);
    for (let y = 0; y < Ht; y++) {
      const v = (y + 0.5) / Ht * 2 - 1;            // across the ribbon
      const across = Math.max(0, 1 - v * v) ** 0.85;
      for (let x = 0; x < Wt; x++) {
        const u = (x + 0.5) / Wt;                   // 0 = head
        const along = (1 - u) ** 1.6;
        const o = (y * Wt + x) * 4;
        data[o] = 255; data[o + 1] = 255; data[o + 2] = 255;
        data[o + 3] = across * along * 255;
      }
    }
    const t = new THREE.DataTexture(data, Wt, Ht, THREE.RGBAFormat);
